...
public abstract class AbstractSortAlgorithm {
     ...
     * 获取有序的抽样数据
     *
     * @param key：配置的缓存对象池redis-prefix
     */
    public List<DynamicHotCacheObj> sampling(String key) {
        Map<String, DynamicHotCacheObj> map = redisUtil.mGetRandom(key, properties.getSamplesNums());
    ...
    public void tryUpdateHotCachePool() {
        // 1.对所有数据进行抽样
        List<DynamicHotCacheObj> sampleList = sampling(properties.getAllKeyPoolPrefix());
        // 2.获取旧的热点缓存池
